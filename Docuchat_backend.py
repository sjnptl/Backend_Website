...
async def get_available_documents():
    """
    Get list of available documents from S3 bucket.

    Returns:
        JSONResponse: A JSON response containing a list of available documents in the S3 bucket.

    Raises:
        HTTPException: If unable to retrieve documents from S3.
    """
    try:
        ...
        if "Contents" in response:
            for obj in response["Contents"]:
                ...
                if filename:  # Skip empty names (e.g., folder paths)
                    documents.append({
                        "name": filename,
                        "path": key,
                        "size": obj["Size"],
        ...
        raise HTTPException(status_code=500, detail="Failed to retrieve documents from S3")
